...
def _ensure_cmd(
    ...
    Note that "already providing both names" is judged by name only, so a
    changed `args` does *not* cause a reload. That is deliberate: reloading
    the module out from under a running capture client silently costs that
    client its echo cancellation — it keeps reading the raw hardware source
    and never re-attaches. Changing args is therefore a manual unload plus a
    restart of whatever was listening, not something to do behind its back.
    """
    sudo = f"sudo -u {pulse_user} "
    xdg = f'XDG_RUNTIME_DIR="/run/user/$(id -u {pulse_user})"'
    pactl = f"{sudo}{xdg} pactl"
    aec_args = f' aec_args="{args}"' if args else ""
    return (
        ...
        f"source_name={source_name} sink_name={sink_name} aec_method={method}{aec_args}"
    )
...
async def ensure_loaded(
    ...
    rc = await executor.run(
        _ensure_cmd(pulse_user, source_name, sink_name, method, args)
    )
    if rc == 0:
